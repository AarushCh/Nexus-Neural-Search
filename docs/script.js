document.addEventListener("DOMContentLoaded", () => {
    ...
    class NeuralNetwork {
        ...
        animate() {
            ...
            this.particles.forEach((p, i) => {
                ...
                if (isLight) { this.ctx.fillStyle = p.color; this.ctx.shadowBlur = 5; this.ctx.shadowColor = p.color; }
                else { this.ctx.fillStyle = '#ffffff'; this.ctx.shadowBlur = 10; this.ctx.shadowColor = '#ffffff'; }
    ...
    window.returnToMain = () => {
        ...
        if (lastSearchData && lastSearchData.length > 0) { renderResults(lastSearchData); if (fb) fb.style.display = 'flex'; }
        else { document.getElementById('results-grid').innerHTML = ''; if (fb) fb.style.display = 'none'; }
    };
    ...
    window.login = async () => {
        ...
        const originalText = btn.innerText; btn.innerText = "CONNECTING..."; btn.disabled = true;
        const u = document.getElementById("auth-username").value;
        const p = document.getElementById("auth-password").value;
        if (!u || !p) { document.getElementById('auth-error').innerText = "MISSING CREDENTIALS"; btn.innerText = originalText; btn.disabled = false; return; }
        try {
            const form = new FormData(); form.append("username", u); form.append("password", p);
            ...
            if (res.ok) {
                ...
                window.closeAuth(); bootSystem(); alert("LOGIN SUCCESSFUL");
            } else { const err = await res.json(); throw new Error(err.detail || "Invalid Credentials"); }
        } catch (e) { document.getElementById('auth-error').innerText = "LOGIN FAILED: " + e.message; }
        finally { btn.innerText = originalText; btn.disabled = false; }
    };

    window.signup = async () => {
        const btn = document.querySelector('#signup-view .primary');
        const originalText = btn.innerText; btn.innerText = "REGISTERING..."; btn.disabled = true;
        ...
        if (p !== c) { document.getElementById('auth-error').innerText = "PASSWORDS DO NOT MATCH"; btn.innerText = originalText; btn.disabled = false; return; }
        try {
    ...
    window.logout = () => { localStorage.removeItem('freeme_token'); localStorage.removeItem('freeme_user'); localStorage.removeItem('freeme_history'); location.reload(); };
    ...
    function renderResults(data, isSimilarView = false) {
        ...
        if (CURRENT_SORT === 'RATING') { filtered.sort((a, b) => (parseFloat(b.rating) || 0) - (parseFloat(a.rating) || 0)); }
    ...
    window.openWishlist = () => {
        ...
        const grid = document.getElementById('results-grid');
        const fb = document.getElementById('filter-bar'); if (fb) fb.style.display = 'none';
        if (!AUTH_TOKEN) { grid.innerHTML = `<h3 style="text-align:center;grid-column:1/-1;color:var(--text-secondary);margin-top:50px;">PLEASE LOGIN TO VIEW WISHLIST</h3>`; return; }
        ...
        fetch(`${API_URL}/wishlist`, { headers: { "Authorization": `Bearer ${AUTH_TOKEN}` } })
            .then(res => res.json()).then(data => {
                if (!data || data.length === 0) { grid.innerHTML = `<h3 style="text-align:center;grid-column:1/-1;margin-top:50px;">YOUR WISHLIST IS EMPTY</h3>`; }
                else { renderResults(data, true); }
